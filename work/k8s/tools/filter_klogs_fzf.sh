...
fzf --ansi --disabled --multi --query "$INITIAL_QUERY" \
    --bind "start:reload:$RG_PREFIX {q} ./klogs/**/*.log" \
    --bind "change:reload:sleep 0.1; $RG_PREFIX {q} ./klogs/**/*.log || true" \
    --bind 'ctrl-a:select-all,ctrl-d:deselect-all' \
    --color 'hl:-1:underline,hl+:-1:underline:reverse' \
    --prompt 'Filter ❯ ' \
    --delimiter : \
    --border-label '   Filter Logs ' | gsed 's/^[^{]*//' | jq -s . > "$tmp" && LIMITED=Y nvim -R -c 'setfiletype json' "$tmp"; rm -f "$tmp"
