-- Kulala helpers for copying HTTP requests as GNU wget commands.
...
--- Find the first argument after the cURL executable.
---@param words string[]
---@return integer|nil first_argument
---@return string|nil err
local function curl_arguments_start(words)
    local index = 1
    ...
    if executable ~= "curl" then
        return nil, ("Expected a curl command, got %s"):format(words[index] or "nothing")
    end
    return index + 1
end
...
--- Translate cURL's no-value header syntax to a Wget-compatible empty header.
---@param header string
---@return string
local function normalize_curl_header(header)
    local name = header:match("^%s*([^:;]+)%s*;%s*:%s*$") or header:match("^%s*([^:;]+)%s*;%s*$")
    name = name and name:match("^%s*(.-)%s*$")
    if name and name:match("^[%w!#$%%&'*+%.%^_`|~-]+$") then
        return name .. ":"
    end
    return header
end
    ...
    while index <= #words do
        ...
        elseif word:sub(1, 2) == "--" then
            ...
            elseif option == "--header" then
                value, index, err = option_value(words, index, inline_value, option)
                if value then
                    table.insert(request.headers, normalize_curl_header(value))
                end
...
function M.curl_to_wget(command, opts)
    ...
    local start_index
    start_index, err = curl_arguments_start(words)
