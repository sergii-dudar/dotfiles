...
local function test_chain_at_cursor()
    ...
    -- Find the innermost `it(...)` call whose range contains the cursor row.
    -- Matching by row range (not a fixed column) makes this work when the cursor
    -- is on the `it(...)` line itself at any column — e.g. when the test-report
    -- tree jumps to col 0 of that line to rerun a test — not only when the cursor
    -- is somewhere inside the test body. `it()` blocks don't nest, so the
    -- smallest matching range is unambiguous.
    local it_node, it_span
    local function scan(n)
        local sr, _, er, _ = n:range()
        if row < sr or row > er then
            return
        end
        if n:type() == "function_call" and is_kind(call_name(n), IT_NAMES) then
            local span = er - sr
            if not it_node or span < it_span then
                it_node, it_span = n, span
            end
        end
...
local function ensure_local_lua_adapter()
    ...
    if vim.fn.isdirectory(LUA_LOCAL_EXT) == 0 then
        return false, "local-lua-debugger-vscode not found at " .. LUA_LOCAL_EXT .. " (install via :Mason)"
    end
...
function M.dap_launch_test(context)
    ...
    -- The selection is stored in the shared `state.last` so it survives across a
    -- regular <-> debug toggle (TOGGLE_LAST_DEBUG / <leader>tD, <leader>tl).
    local t = context.test_type
    local root = M.project_root()
    ...
    elseif t == task.test_type.CURRENT_TEST then
        ...
        state.last = { kind = "single", spec_paths = spec_paths, filter = filter, chain = chain, test_type = t }
    elseif t == task.test_type.FILE_TESTS then
        ...
        spec_paths = { file }
        state.last = { kind = "file", spec_paths = spec_paths, test_type = t }
    else
