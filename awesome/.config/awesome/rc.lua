...
require("modules.widgets.init").setup()
...
local theme_path = string.format("%s/.config/awesome/themes/%s/theme.lua", vars.path.home_dir, choosen_theme)
...
clientkeys = gears.table.join(
    awful.key({ vars.key.modkey }, "f", function(c)
        c.fullscreen = not c.fullscreen
        c:raise()
    end, { description = "toggle fullscreen", group = "client" }),
    awful.key({ vars.key.modkey, "Shift" }, "c", function(c)
        c:kill()
...
--     awful.spawn.with_shell("~/dotfiles/bin/wmscripts/autostart_once.sh")
-- end
